...
namespace GlobusT
{
    public partial class RequestCard : UserControl
    {
        ...
        private void RequestCard_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            if (DataContext is Request request && request.IdStatusRequestNavigation != null)
            {
                var statusBorder = this.FindName("StatusBorder") as Border;
                if (statusBorder != null)
                {
                    statusBorder.Background = GetStatusColor(request.IdStatusRequestNavigation.Name);
                }
            }
        }

        private Brush GetStatusColor(string statusName)
        {
            ...
            {
                "новая" => new SolidColorBrush(Color.FromRgb(52, 152, 219)),     // Синий
                "в работе" => new SolidColorBrush(Color.FromRgb(241, 196, 15)),  // Желтый
                "завершена" => new SolidColorBrush(Color.FromRgb(46, 204, 113)), // Зеленый
                "отменена" => new SolidColorBrush(Color.FromRgb(231, 76, 60)),   // Красный
